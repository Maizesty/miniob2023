...
FieldMeta::FieldMeta(const char *name, AttrType attr_type, int attr_offset, int attr_len, bool visible, bool isNullable)
{
...
RC FieldMeta::init(const char *name, AttrType attr_type, int attr_offset, int attr_len, bool visible, bool isNullable)
{
...
bool FieldMeta::isNullable() const{
  return isNullable_;
}
...
RC FieldMeta::from_json(const Json::Value &json_value, FieldMeta &field)
{
  ...
  if (!isNullable_value.isBool()) {
    LOG_ERROR("Isnullable field is not a bool value. json value=%s", isNullable_value.toStyledString().c_str());
    return RC::INTERNAL;
  }
  ...
  return field.init(name, type, offset, len, visible,isnullable);
}
